...
% The optical current injection $\mathbf{\dot{J}}(\omega)$, is a surface-sensitive optical effect that offers the potential to control chemical reactions \cite{bhatPRB05,hachePRL97}. 
...
% generadores eficientes de SHG y caracterizacion de estructuras de este tipo
...
In a layer by layer analysis, we find that the components \emph{alt} structure
are primarily generated in the carbon plane, with some contribution from the
upper H atom. The lower H atom provides a very small portion of the total
signal. The spectra for the \emph{up} structure is completely dominated by the
...
therefore an excellent candidate for SHG.
...
\begin{figure}[t]
\subfloat{\includegraphics[width=\linewidth]{figures/shg-vnl-up}}\\
\caption{(Color online) The three largest components of
