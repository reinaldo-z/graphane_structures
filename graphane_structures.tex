...
\end{equation}
where, for both structures considered here, $\xi^{ab}(\omega)$ is diagonal and the superscripts denote Cartesian directions. 
...
layered system we have that the total contribution to $ \eta^{abc}(\omega)$ is given by
...
\end{equation*} where $\chi^{abc}(-2\omega;\omega,\omega)$ is the nonlinear susceptibility tensor responsible for the SHG that satisfies permutation symmetry condition and, as mentioned before,  if the superscripts are repeated, then are to be summed over. 
Analogously to the case of optical current injection we have that for a layered system the total contribution for the $\chi^{abc}(-2\omega;\omega,\omega)$ is given by
\begin{equation}\label{eq:chisum}
\chi^{abc}_{\mathrm{Total}}(-2\omega;\omega,\omega)=
\ell_{\mathrm{eff}}\sum_{\ell=1}^{N_{\mathrm{eff}}}\chi^{abc}(\ell|-2\omega;\omega,\omega),
\end{equation}
where $\chi^{abc}(\ell|-2\omega;\omega,\omega)$ gives the contribution from the corresponding
$\ell^{\mathrm{th}}$ layer to the nonlinear susceptibility tensor. The sum in Eq.
\eqref{eq:chisum} is carried out from the top layer, $\ell=1$, to
$\ell=N_{\mathrm{eff}}$, that is the resulting values for the tensor components
have contributions from the $N_{\mathrm{eff}}$ layers that extend to an
effective length, $\ell_{\mathrm{eff}}$. The expressions for the imaginary part of the tensor components $\mathrm{Im}[\chi^{abc}(-2\omega;\omega,\omega)]$ for $1\omega$ and $2\omega$ are divided into interband and intraband contributions including the contribution from the nonlocal part of the pseudopotentials and the scissors correction and the corresponding real part can be obtained through the Kramers-Kronig transformation  \cite{andersonPRB15}. Finally, the total contribution for the nonlinear susceptibility tensor can be calculated by 
...
In Table \ref{tab:dacomp} we present a comparison of the absolute maximum values
of $\mathcal{D}^{a}(\omega)$ in a given direction for different materials and
the corresponding direction and energy at which they are reached. From this
table we have that the \emph{alt} and \emph{up} systems have less DSP than the
Si(111)-As $1\times1$ and the the bulk CdSe cases, but are comparable to the
Si(111)-In $8\times2$, bulk Si, and bulk GaAs systems. Si(111)-In $8\times2$ and
\emph{alt} both have maxima around 0.72\,eV, which is in the near infrared
range. Conversely, the maxima for Si(111)-As $1\times1$, bulk Si, and bulk GaAs
all occur in the optical energy range. The 0.7\,eV maxima for \emph{alt} makes
it an interesting candidate for further study, as this energy is readily
obtainable using terahertz radiation.
\begin{table}[b]
\sidecaption
\begin{tabular}{lcccc}
\hline
...
three layers contribute toward $\eta^{xxy}(E_{1})$ with the carbon
and top H layers providing the principal contributions. For $\eta^{xxy}(E_{2})$, the
dominating layers are the carbon and bottom H layers with minimal contribution
from the top H layer. For $\eta^{yxy}(E_{1})$ component is also
...
$\eta^{yxy}(E_{2})$ the spectrum is completely dominated by the bottom H layer, with
almost no contribution from the others. Lastly 
$\eta^{zxy}(E_{1})$ component is entirely produced from the carbon layer,
since the H layers provide no response whatsoever. This component is negligible
at $E_{2}$ and note that the response for all three components vanishes after
1.75\,eV. Using Eq. \eqref{eq:etatotal}, we find that the absolute current
injection $\eta(E_{1})$ is
$5.10\,\mathrm{mC}^{3}/\mathrm{J}^{2}\mathrm{s}^{2}$ with angles
$\varphi=116^{\circ}$ and $\theta=-172^{\circ}$. For $\eta(E_{2})$, we find that
...
$\eta(\omega)$ $\eta(E_{1})$ and $\eta(E_{2})$. As mentioned
...
0.09\,\mathrm{eV}$ and $E_{2} = 0.10\,\mathrm{eV}$. For $\eta^{axy}(E_{1})$, the carbon
layer dominates the total response for all three components. 
For $\eta^{yxy}(E_{2})$ and $\eta^{zxy}(E_{2})$ the
carbon layer dominates the response of the system,
but both the H and carbon layer contribute significantly to
$\eta^{xxy}(E_{2})$. Identically to the \emph{alt} structure, the
...
\eqref{eq:etatotal}, we find that the absolute current injection $\eta(E_{1})$
is $15.12\,\mathrm{mC}^{3}/\mathrm{J}^{2}\mathrm{s}^{2}$ with angles
$\varphi=58^{\circ}$ and $\theta=-54^{\circ}$. Also, we find that
$\eta(E_{2})=7.62\,\mathrm{mC}^{3}/\mathrm{J}^{2}\mathrm{s}^{2}$ with angles
