...
\cite{mendoza2012optical,nastos2007full} for bulk, surfaces, and interfaces, 
...
We assume an incoming beam of CPL at normal incidence that
propogates along the $-z$ direction, $\mathbf{E}(\omega) = E_{0}(\hat{x} -
i\hat{y})/\sqrt{2}$. The DSP along a given $a$ direction is defined as $D^{a}=\dot{S}^{a}(\omega)/(\hbar/2)\dot{n}(\omega)$ and then 
\end{changed}
...
\begin{changed}
\end{changed} 
From Eq. \ref{eq:D^i} we have that the spin
injection tensor is given by
